...
class RoundedCard(tk.Canvas):
    def __init__(self, master, *, theme, scale, radius=12, padding=12, **kwargs):
        ...
        self._shape = self.create_polygon(
            *rounded_rectangle_points(0, 0, 2, 2, 1),
            smooth=True,
            splinesteps=24,
    ...
    def set_interactive_state(self, *, hovered=False, focused=False):
        ...
        background = (
            self.theme["surface_hover"] if self._hovered else self.theme["surface_1"]
        )
        border = (
            self.theme["border_focus"]
            if self._focused
            else (
                self.theme["border_hover"]
                if self._hovered
...
class SearchField(tk.Canvas):
        ...
        *,
        ...
        self._shape = self.create_polygon(
            *rounded_rectangle_points(
                ...
                1,
                100,
                self.s(METRICS.search_height) - 1,
                self.s(METRICS.input_radius),
            ),
            smooth=True,
            splinesteps=24,
            fill=theme["surface_1"],
    ...
    def _on_configure(self, event):
        ...
        self.itemconfigure(self._entry_window, width=max(1, width - self.s(74)))
...
class SegmentedControl(tk.Canvas):
    def __init__(self, master, *, options, variable, command, theme, scale, height=32):
        ...
        self._trace_id = self.variable.trace_add(
            "write",
            lambda *_args: self._sync_from_variable(),
        )
    ...
    def _clicked(self, event):
        ...
        index = min(
            len(self.options) - 1,
            max(0, int(event.x * len(self.options) / width)),
        )
    ...
    def _render(self):
        ...
        for index, (value, text) in enumerate(self.options):
            ...
            x2 = max(x1 + 1, round((index + 1) * segment_width) - self.s(2))
            if value == selected:
                self.create_polygon(
                    *rounded_rectangle_points(
                        x1,
            ...
            self.create_text(
                ...
                font=(
                    "Segoe UI Semibold" if value == selected else "Segoe UI",
                    8,
                ),
                tags="segment",
            )
...
class ToggleSwitch(tk.Canvas):
    ...
    def set_value(self, value, *, emit=False):
        value = bool(value)
        self.state_model.set(value, emit=False)
        self.variable.set(value)
        self._render()
        if emit:
            self.command(value)
    ...
    def _render(self):
        ...
        track = self.theme["purple"] if enabled else self.theme["border_hover"]
        self.create_oval(1, 1, height - 1, height - 1, fill=track, outline="")
        self.create_oval(
            width - height + 1,
            1,
            width - 1,
...
class ThemedSlider(tk.Canvas):
    ...
    def _user_set(self, event):
        ...
        fraction = max(
            0.0,
            min(1.0, (int(event.x) - self.s(6)) / width),
        )
    ...
    def _render(self):
        ...
        height = max(2, int(self.winfo_height()))
        left = self.s(6)
        right = max(left + 1, width - self.s(6))
        center = height // 2
...
class OverlayScrollbar(tk.Canvas):
    ...
    def _press(self, event):
        ...
        if top <= int(event.y) <= bottom:
            ...
            fraction = max(
                0.0,
                min(1.0, int(event.y) / max(1, int(self.winfo_height()))),
            )
