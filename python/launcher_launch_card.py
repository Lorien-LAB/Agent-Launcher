...
class LaunchOptionsCard(CleanRoundedCard):
    def __init__(self, master, *, theme, scale, callbacks):
        ...
        self.tab_hint.grid(row=2, column=0, sticky="ew", pady=(scale(2), 0))

        toggles = tk.Frame(body, bg=theme["surface_1"])
        toggles.grid(row=3, column=0, sticky="ew", pady=(scale(4), 0))
        ...
        tk.Label(toggles, text="Hide Launcher after launch", bg=theme["surface_1"], fg=theme["text_secondary"], anchor="w", font=("Segoe UI", 9)).grid(row=1, column=0, sticky="ew", pady=(scale(3), 0))
        self.hide_toggle = ToggleSwitch(toggles, variable=self.hide_var, command=lambda _value: self._changed(), theme=theme, scale=scale)
        self.hide_toggle.grid(row=1, column=1, padx=(scale(8), 0), pady=(scale(3), 0))
        ...
        self.claude_button = GlassBrandButton(buttons, role="claude", text="Claude Code", command=lambda: callbacks.on_launch("claude"), theme=theme, scale=scale, width=135, height=34)
        self.claude_button.grid(row=0, column=0, sticky="ew", padx=(0, scale(4)))
        self.hermes_button = GlassBrandButton(buttons, role="hermes", text="Hermes", command=lambda: callbacks.on_launch("hermes"), theme=theme, scale=scale, width=115, height=34)
